...
class ProductService(Singleton):
    ...
    def register_product(self, product: ProductRegisterDTO, requester_id: int):
        ...
        if product.batches:
            saved_product.batches = self.batch_service.register_batch(product.batches, saved_product.id)

        return saved_product

    def get_products(self):
        products = self.product_repository.find_all()
        return [ProductRegisteredDTO(product) for product in products]
    ...
    def update_product(self, product_id: int, product: ProductUpdateDTO, requester_id: int):
        ...
        product_found.isActive = product.is_active if product.is_active else product_found.isActive
